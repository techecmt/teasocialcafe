...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    let disposed = false;
    // Defer one frame so React Strict Mode's mount→unmount→mount cycle doesn't
    // create a WebGL context and immediately lose it (forceContextLoss) on the
    // same canvas — which leaves getContext returning null on the remount.
    const raf = requestAnimationFrame(() => {
      ...
      try {
        ...
        // No WebGL / context creation failed — fail quietly so the page still
        // renders (the gradient background remains visible).
        console.warn('Ballpit: WebGL unavailable, skipping animation.', err);
      }
    });
